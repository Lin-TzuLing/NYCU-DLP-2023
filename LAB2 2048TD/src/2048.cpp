 ...
 * 16進位(0x)，按照格子編號(index)放各格裡面的數字(以2的幾次方表示)，ull (unsigned long long)
...
public:
	...
	void popup() {
		...
		if (num)
			// choose empty blank (rand()%num), and set value 2^1 or 2^2 (rand()%10)
	...
	/**
	 * SELF_NEW function
	 * return index of empty blanks on the board
	 */
	std::vector<int> get_empty_idx(){

		std::vector<int> empty_idx;
		
		// search 16 blanks for empty blank, and push_back its index into vector
		for (int i=0; i<16; i++){
			if (at(i)==0){
	...
	/**
	 * SELF_NEW function
	 * popup a specific tile at specific idx on the board
	 * int pop_idx = where to popup specific tile
	 * int pop_power = new tile equals 2^(power)
	 */
	void popup_specific(int pop_idx, int pop_power) {
		
		set(pop_idx, pop_power);
...
public:
	...
	virtual float estimate(const board& b) const {
		...
		/* value = sum of 4(rotate)*2(flip, mirror) isomorphics' weights */
		for (int i=0; i<iso_last; i++){
			// get idx of isomorphic pattern
			size_t idx = indexof(isomorphic[i], b);
			// add weight (find by idx) to total board value
			// operator[]：returns weight[i]
			value += operator[](idx); 
		}
	...
	virtual float update(const board& b, float u) {
		...
		/* split board total update value u to each isomorphic */
		float update_isomorphic = u / iso_last;
		float new_value = 0;

		for (int i=0; i < iso_last; i++){
			size_t idx = indexof(isomorphic[i], b);
			// weight of idx (index of isomorphic pattern) increase by (update_each) 
			operator[](idx) += update_isomorphic;
			// calculate new value of the whole board
			new_value += operator[](idx);
		}
...
protected:
	...
	size_t indexof(const std::vector<int>& patt, const board& b) const {
		...
		/* get pattern idx in board b */
		for (size_t i=0; i < patt.size(); i++){
			// b.at(patt[i]) = pattern idx in board b, shift left by 4 bits for a blank in board
			// idx = b.patt[n] | b.patt[n-1] | ... | b.patt[1] | b.patt[0] (4 bits for a index)
			idx |= b.at(patt[i]) << (4*i);
		}
...
public:
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				...
				/* consider new tile in next board */
				std::vector<int> empty_idx = s.get_empty_idx();
				// sum up value of all possible new boards with probability as weight 
				float value = 0;
				// probability of a new tile (2/4) popup at specific empty_idx
				float prob_2 = 0.9f / empty_idx.size();
				float prob_4 = 0.1f / empty_idx.size();
				

				for (int i=0; i<empty_idx.size(); i++){
					// two possible boards after s, popup 2/4 at specific empty_idx i 
					board s_next_2 = s;
					board s_next_4 = s;

					// s_next_2 and s_next_4 popup 2/4 at empty_idx[i], respectively
					s_next_2.popup_specific(empty_idx[i], 1);
					s_next_4.popup_specific(empty_idx[i], 2);
					
					// sum up possible new boards with probability as weight 
					value += prob_2 * (estimate(s_next_2));
					value += prob_4 * (estimate(s_next_4));
				}

				// value of this move = reward of this move + prob weighted sum of new boards' value 
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		...
		/* update each V(s) with TD error, s=before_state
		* V(s) <- V(s) + alpha * {[reward' + V(s')] - V(s)}
		* TD target = [reward + V(s')]
		* TD error = {[reward + V(s')] - V(s)}
		* V(s_last) always equals to 0, initial value_s_next=0
		*/

		// save for update, valud_s_next = new value of V(s') that has been updated 
		float value_s_next = 0;
		while(!path.empty()){
			// step = the last (before state, after state, action, reward) element in <vector> &path
			state &step = path.back();
			// td error = reward + V(s') - V(s)
			float td_error = (step.reward() + value_s_next) - estimate(step.before_state());
			value_s_next = update(step.before_state(), alpha*td_error);
			// pop current step from the <vector> &path
			path.pop_back();
		}
	...
	std::vector<float> make_statistic(size_t n, const board& b, int score, int unit = 1000) {
		...
		// SELF_NEW save result in <vector> and return 
		std::vector<float> result;
		result.push_back(mean);
		result.push_back(max);
		result.push_back(win_rate_2048);
		return result;
...
int main(int argc, const char* argv[]) {
	...
	std::string exp_name = "tuple_type"; 
	std::string lr = "0.1";
	// std::string tuple_type = "original";
	// std::string tuple_type = "type1";
	std::string tuple_type = "type2";
	// std::string tuple_type = "type3";
	// std::string tuple_type = "type4";
	bool train_load_flag = false; 

	transform(mode.begin(), mode.end(), mode.begin(), ::tolower);
	transform(exp_name.begin(), exp_name.end(), exp_name.begin(), ::tolower);
	transform(tuple_type.begin(), tuple_type.end(), tuple_type.begin(), ::tolower);
	...
	if (tuple_type=="original"){
		info << "tuple = " << tuple_type << std::endl;
	...
	if(mode=="train"){
		...
		info << "\n2048 Training...\n";
		if (train_load_flag == true){
			tdl.load("../save_model/" + tuple_type + "_" + lr + "_best.bin");
			float best_win_rate_2048 = 0;
		}
		...
		std::string result_path("../result/" + tuple_type + "_" + lr + ".txt");
		std::ofstream result_file;
		result_file.open(result_path);
		if (!result_file.is_open()) {
			info << "Fail to open result file!" << std::endl;
			return 0;
		}
		result_file << "training\tlr="+lr+"\ttupleType="+tuple_type+"\n";
		result_file << "episode\tmean\tmax\twin_rate_2048\n";
		...
		for (size_t n = 1; n <= total; n++) {
			...
			std::vector<float> statistic = tdl.make_statistic(n, b, score);
			//SELF_NEW
			result_file << n << "\t" << statistic[0] << "\t" << statistic[1] << "\t" << statistic[2] << std::endl;
			path.clear();		

			// Save the model based on best 2048 win rate
			if (n % report_every == 0) {
				if (statistic[2] > best_win_rate_2048) {
					info << "\nSave model (best win_rate_2048)...\n";
					best_win_rate_2048 = statistic[2];
					// store the model into file
					tdl.save("../save_model/" + tuple_type + "_" + lr + "_best.bin");
				}
			}	
		}
		result_file.close();
	...
	if(mode=="demo"){
		...
		// Create statistic file
		std::string result_path("../result/demo_" + tuple_type + "_" + lr + ".txt");
		std::ofstream result_file;
		result_file.open(result_path);
		if (!result_file.is_open()) {
			info << "Fail to open result file!" << std::endl;
		...
		result_file << "demo\tlr="+lr+"\ttupleType="+tuple_type+"\n";
		result_file << "episode\tmean\tmax\twin_rate_2048\n";
		...
		for (size_t n = 1; n <= total; n++) {
			...
			std::vector<float> demo_statistic = tdl.make_statistic(n, b, score, report_every);
			//SELF_NEW
			result_file << n << "\t" << demo_statistic[0] << "\t" << demo_statistic[1] <<
			 "\t" << demo_statistic[2] << std::endl;
			path.clear();	
		}
		return 0;
	}
