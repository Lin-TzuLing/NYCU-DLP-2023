    ...
    for episode in range(args.episode):
        ...
        for t in itertools.count(start=1):
            ...
            if total_steps < args.warmup:
                ...
                action = agent.select_action(state).detach().cpu().numpy()
            # execute action
...
def test(args, env, agent, writer):
    ...
    for n_episode, seed in enumerate(seeds):
        ...
        with torch.no_grad():
            for t in itertools.count(start=1):
                # select action
                action = agent.select_action(state, noise=False).detach().cpu().numpy()
                # execute action
                next_state, reward, done, _ = env.step(action)
...
def main():
    ...
    parser.add_argument('-m', '--model', default='../checkpoint/ddpg_new.pth')
