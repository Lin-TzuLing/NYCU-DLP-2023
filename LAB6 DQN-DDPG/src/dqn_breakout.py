...
class DQN:
    ...
    def select_action(self, state, epsilon, action_space):
        ...
        if random.random() > epsilon:
            with torch.no_grad():
                input_state = torch.from_numpy(np.array(state)).to(self.device)
                input_state = input_state.permute([2,0,1]).unsqueeze(dim=0)
    ...
    def append(self, state, action, reward, next_state, done):
        ...
        a = state.__array__()
        state = np.transpose(np.array(state), axes=(2,0,1))
        next_state = np.transpose(np.array(next_state), axes=(2,0,1))
...
def main():
    ...
    parser.add_argument('-tmp', '--test_model_path', default='../ckpt2/dqn_4800000.pt')
